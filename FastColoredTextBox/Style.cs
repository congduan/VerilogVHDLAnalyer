...
namespace FastColoredTextBoxNS
{
    ...
    public class TextStyle : Style
    {
        ...
        public override void Draw(Graphics gr, Point position, Range range)
        {
            ...
            Line line = range.tb[range.Start.iLine];//范围的开始行
            float dx = range.tb.CharWidth;//字符宽度
            float y = position.Y + range.tb.LineInterval / 2;//加上行距的一半
            float x = position.X - range.tb.CharWidth/3;//减去字符宽度的1/3

            if(ForeBrush == null)
                ForeBrush = new SolidBrush(range.tb.ForeColor);

            //IME 模式(此处渲染中文，原来的方式比较卡)
