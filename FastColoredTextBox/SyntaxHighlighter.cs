...
namespace FastColoredTextBoxNS
{
    public class SyntaxHighlighter: IDisposable
    {
        ...
        public virtual void HighlightSyntax(Language language, Range range)
        {
            switch (language)
            {
                case Language.Verilog: VerilogSyntaxHighlight(range); break;
        ...
        private void CSharpAutoIndentNeeded(object sender, AutoIndentEventArgs args)
        {
            ...
            //some statements: case, default 匹配case语句
    ...
    brackets.Attributes["left2"].Value == "" || brackets.Attributes["right2"].Value == "")
        ...
        #region Verilog
        Regex VerilogStringRegex, VerilogCommentRegex1,VerilogCommentRegex2,VerilogCommentRegex3, 
            VerilogNumberRegex, VerilogModuleRegex, VerilogKeywordRegex ,VerilogParamRegex,VerilogUppercaseRegex;

        /// <summary>
        /// 初始化Verilog正则表达式
        /// </summary>
        void InitVerilogRegex()
        {
            VerilogStringRegex = new Regex(@"", RegexCompiledOption);
            VerilogCommentRegex1 = new Regex(@"//.*$", RegexOptions.Multiline | RegexCompiledOption);
            VerilogCommentRegex2 = new Regex(@"(/\*.*?\*/)|(/\*.*)", RegexOptions.Singleline | RegexCompiledOption);
            VerilogCommentRegex3 = new Regex(@"(/\*.*?\*/)|(.*\*/)", RegexOptions.Singleline | RegexOptions.RightToLeft | RegexCompiledOption);
            VerilogNumberRegex = new Regex(@"(\b\d+\'[ha-f0-9]+\b)|(\[\d+\:\d+\])", RegexCompiledOption);
            VerilogUppercaseRegex = new Regex(@"\b[A-Z]+[A-Z_0-9]*\b", RegexCompiledOption);
            VerilogModuleRegex = new Regex(@"\bmodule\s+(?<range>\w+?)\b", RegexCompiledOption);
            VerilogKeywordRegex = new Regex(@"\b(reg|wire|if|posedge|always|ifnone|rnmos|and|incdir|rpmos|assign|include|rtran|automatic|initial|rtranif0|begin|inout|rtranif1|buf|input|scalared|bufif0|instace|showcancelled|bufif1|integer|signed|case|join|small|casex|large|spcify|casez|liblist|specpara|cell|library|strong0|cmos|localparam|strog1|config|macromodule|supply0|deassign|medium|supply1|default|module|table|defparam|nand|task|design|negedge|time|disable|nmos|tran|edge|nr|tranif0|else|noshowcancelled|tranif1|end|not|tri|endcase|notif0|tri0|endconfig|notif1|tri1|endfunction|or|triand|endgenerate|output|trior|endmodule|parameter|trireg|endprimitive|pmos|unsigned|endspecify|poedge|use|endtable|primitive|vectored|endtask|pull0|wait|event|pull1|wnd|for|pulldown|weak0|force|pullup|weak1|forever|pulsestyle_onevent|while|fork|pulsestyle_ondetect|wire|function|rcmos|wor|generate|real|xnorgenvarrealtimexorhigh0reghighz1releaseifrepeat)\b|#region\b|#endregion\b", RegexCompiledOption);
            VerilogParamRegex = new Regex(@"\`\b\w\b", RegexCompiledOption);
        }
        ...
        /// </summary>
        /// <param name="range"></param>
        public virtual void VerilogSyntaxHighlight(Range range)
        {
            range.tb.CommentPrefix = "//";
            range.tb.LeftBracket = '(';
            range.tb.RightBracket = ')';
            ...
            range.tb.RightBracket2 = '\x0';
            //clear style of changed range
            range.ClearStyle(StringStyle, CommentStyle, NumberStyle, AttributeStyle, ClassNameStyle, KeywordStyle, VariableStyle, KeywordStyle2);
            //
            if (VerilogStringRegex == null)
                InitVerilogRegex();
            range.SetStyle(VariableStyle, VerilogParamRegex);
            range.SetStyle(KeywordStyle2, VerilogUppercaseRegex);
            //string highlighting
            range.SetStyle(StringStyle, VerilogStringRegex);
            //comment highlighting
            range.SetStyle(CommentStyle, VerilogCommentRegex1);
            range.SetStyle(CommentStyle, VerilogCommentRegex2);
            range.SetStyle(CommentStyle, VerilogCommentRegex3);
            //number highlighting
            range.SetStyle(NumberStyle, VerilogNumberRegex);
            //class name highlighting
            range.SetStyle(ClassNameStyle, VerilogModuleRegex);
            //keyword highlighting
            range.SetStyle(KeywordStyle, VerilogKeywordRegex);            

            //clear folding markers
            range.ClearFoldingMarkers();
            //set folding markers
            range.SetFoldingMarkers(@"begin\b", @"end\b");//allow to collapse brackets block
            range.SetFoldingMarkers(@"module\b", @"endmodule\b");//allow to collapse brackets block
            range.SetFoldingMarkers(@"/\*", @"\*/");//allow to collapse comment block
        }
        #endregion
    ...
    {
        Custom, CSharp, VB, HTML, SQL, PHP, JS, Verilog
    }
}
